...
agenda_staff(a001, 20241028, [(720, 790, m01), (900, 960, a01)]).
agenda_staff(a002, 20241028, [(850, 900, a02), (1080, 1140, a02)]).
agenda_staff(n001, 20241028, [(720, 790, n01), (800, 860, n01)]).
...
staff(a001,assistant,orthopaedist,[so2,so3,so4]).
staff(a002,assistant,orthopaedist,[so2,so3,so4]).
...
assignment_staff(so100005, a001).
...
availability_all_surgeries([OpCode|LOpCode],Room,Day):-
    ...
    availability_operation(OpCode,Room,Day,LPossibilities,LDoctors,LStaff),
...
availability_operation(OpCode,Room,Day,LPossibilities,LDoctors,LStaff):-surgery_id(OpCode,OpType),surgery(OpType,_,TSurgery,_),
...
obtain_better_sol(Room,Day,AgOpRoomBetter,LAgDoctorsBetter,TFinOp,T):-
    get_time(Ti),
