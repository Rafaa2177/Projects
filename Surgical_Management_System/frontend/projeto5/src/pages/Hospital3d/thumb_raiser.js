...
import { generalData, mazeData, bedData, lightsData, cameraData } from "./default_data.js";
...
export default class ThumbRaiser {
    constructor(generalParameters, mazeParameters, playerParameters, lightsParameters, fogParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters,bedParameters,appointments) {
        ...
        this.maze = new Maze(this.mazeParameters,this.fixedViewCamera,this.appointments);
    ...
    dispose() {
        ...
        // Libera as geometrias, materiais e texturas na cena 3D
        if (this.scene3D) {
            this.scene3D.traverse((object) => {
                ...
                if (object.material) {
                    // Se o material tiver uma textura, também será liberada
        ...
        // Libera as geometrias, materiais e texturas na cena 2D (se houver)
        if (this.scene2D) {
        ...
        // Libera o cubemap de fundo
        if (this.scene3D.background) {
            if (this.scene3D.background instanceof THREE.CubeTexture) {
                this.scene3D.background.dispose();
            }
        }

        // Libera objetos específicos como o labirinto e as luzes (se houver)
        if (this.maze) {
            // Aqui, depende de como a classe Plant (labirinto) gerencia seus objetos.
            if (this.maze.dispose) this.maze.dispose(); // Caso tenha método dispose() na classe Plant
        }

        if (this.lights) {
            if (this.lights.dispose) this.lights.dispose(); // Caso tenha método dispose() na classe Lights
        }

        // Libera as câmeras
        if (this.fixedViewCamera) {
            if (this.fixedViewCamera.dispose) this.fixedViewCamera.dispose(); // Caso a câmera tenha o método dispose()
        }
        if (this.miniMapCamera) {
            if (this.miniMapCamera.dispose) this.miniMapCamera.dispose(); // Caso a câmera tenha o método dispose()
        }

        // Verifica se o renderer DOM existe e se o elemento onde ele foi montado ainda contém o DOM
        if (this.renderer && this.renderer.domElement && this.renderer.domElement.parentNode) {
            this.renderer.domElement.parentNode.removeChild(this.renderer.domElement);
        }

        // Resetar qualquer outro recurso, se necessário, que tenha sido alocado
        // Por exemplo, os eventos registrados, se você os tivesse de forma explícita
        ...
        this.vertical.removeEventListener("change", this.elementChange);
        this.distance.removeEventListener("change", this.elementChange);
        this.zoom.removeEventListener("change", this.elementChange);
        this.userInterfaceCheckBox.removeEventListener("change", this.elementChange);
        this.reset.removeEventListener("click", this.buttonClick);
    }
    ...
    update() {
        if (!this.gameRunning) {
            if (this.maze.loaded) { // If all resources have been loaded
                ...
                this.userInterface = new UserInterface(this.scene3D, this.renderer, this.lights/*, this.player.object, this.animations*/);
