...
export default class Camera {
    ...
    setTopDownView(target) {
        // Salvar a vista atual antes de mudar para top-down
        this.previousView = {
            ...
            initialTarget: this.target.clone()
        };

        // Definir posição final desejada para a vista top-down
        const targetPosition = new THREE.Vector3(target.x, target.y + 5, target.z);
        const targetOrientation = new Orientation(0, -90);

        // Criar objeto com valores iniciais para a animação
        const initial = {
            posX: this.perspective.position.x,
            posY: this.perspective.position.y,
            posZ: this.perspective.position.z,
            targetX: this.target.x,
            targetY: this.target.y,
            targetZ: this.target.z,
            orientH: this.orientation.h,
            orientV: this.orientation.v,
            zoom: this.zoom,
            distance: this.distance
        };

        // Criar objeto com valores finais para a animação
        const final = {
            ...
            targetY: target.y,
            targetZ: target.z,
            orientH: targetOrientation.h,
            orientV: targetOrientation.v,
            zoom: 2,
            distance: 5
        };
        ...
        new TWEEN.Tween(initial)
            .to(final, 1000) // 1000ms = 1 segundo de duração
            .easing(TWEEN.Easing.Quadratic.InOut)
            .onUpdate(() => {
                // Atualizar posição da câmera
                this.perspective.position.set(initial.posX, initial.posY, initial.posZ);

                // Atualizar target
                this.target.set(initial.targetX, initial.targetY, initial.targetZ);

                // Atualizar orientação
                this.orientation = new Orientation(initial.orientH, initial.orientV);

                // Atualizar zoom e distance
                this.zoom = initial.zoom;
                this.distance = initial.distance;

                // Atualizar os parâmetros da câmera
                this.setViewingParameters();
                this.setProjectionParameters();
            })
            .start();
    ...
    resetToDefaultView() {
        ...
        // Criar objeto com valores finais (valores anteriores salvos)
        const final = {
            posX: this.previousView.position.x,
            posY: this.previousView.position.y,
            posZ: this.previousView.position.z,
            targetX: this.previousView.initialTarget.x,
            targetY: this.previousView.initialTarget.y,
            targetZ: this.previousView.initialTarget.z,
            orientH: this.previousView.orientation.h,
            orientV: this.previousView.orientation.v,
            zoom: this.previousView.zoom,
            distance: this.previousView.distance
        };
